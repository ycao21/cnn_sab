...
Table~\ref{tab:main_results} compares CASE against all baselines across four datasets. Among offline baselines, TIFUKNN is the strongest competitor: it achieves the best Precision@1 on Instacart (0.549 vs.\ CASE's 0.534) and is competitive on DC and Walmart. However, TIFUKNN's strong offline performance comes at a fundamental scalability cost: the method must compute user-to-user similarities at inference time, requiring $O(|\mathcal{U}|)$ operations per query. At tens of millions of users this is completely infeasible without approximate nearest neighbor indexes that introduce additional serving infrastructure and degrade retrieval quality. CASE, by contrast, uses a single shared forward pass with no per-user parameters, enabling batch inference at arbitrary scale.

On TaFeng, TIFUKNN underperforms both DNNTSP and CASE across all $K$. With only 6.58 baskets per user on average, there is insufficient history to reliably estimate user similarity, and neighbor-based aggregation degrades. CASE's shared calendar-time encoding is more robust in this regime, operating on absolute purchase timing rather than basket counts and relying on population-level temporal patterns learned from the full training set.

CASE consistently outperforms PersonalTop and DNNTSP across all datasets and all $K$. PersonalTop is a strong frequency baseline but cannot capture timing: it scores an item the same whether it was last purchased yesterday or six months ago. DNNTSP's basket-index representation loses the actual elapsed time between purchases, and as a result underperforms PersonalTop at low $K$ where cadence timing is decisive. CASE combines personalized cadence encoding through the calendar-time CNN with cross-item co-purchase context through ISAB, outperforming both signals independently. 
