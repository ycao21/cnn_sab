...
\textbf{Set Attention (ISAB) provides consistent gains, especially at larger $K$.} Removing the set encoder reduces performance across all $K$ values, with the effect visible at $K{=}10$: Recall@10 drops from 0.546 to 0.540 and NDCG@10 from 0.524 to 0.515. This suggests that ISAB captures co-purchase context that becomes more important as we expand the recommendation list, since at larger $K$ the temporal cadence signal alone may not distinguish between multiple items at similar phases in their repurchase cycle.
