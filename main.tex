...
We compare against four baselines. \textbf{PersonalTop} ranks each user’s history by personal purchase frequency — a strong non-parametric repurchase baseline. \textbf{TIFUKNN}~\cite{hu2020modeling} models personalized item frequency with temporal decay, aggregating scores from a user’s own history and their KNN neighbors in user space. While competitive offline, its $O(|\mathcal{U}|{\cdot}N)$ per-query inference cost is infeasible at production scale. \textbf{DNNTSP}~\cite{yu2020predicting} uses GNN aggregation over an item co-occurrence graph with temporal attention over basket embeddings. \textbf{PIETSP}~\cite{ranjan2025scalable} extends temporal set prediction with permutation-equivariant architectures for improved efficiency. Both DNNTSP and PIETSP use basket-index time rather than calendar days.
...
Table~\ref{tab:main_results} compares CASE against all baselines across four datasets. We discuss performance patterns dataset by dataset, relating them to the model's structural properties.

\textbf{Instacart.} TIFUKNN leads at $K{=}1$ (0.549 vs.\ CASE 0.534) but CASE takes over at $K{\geq}5$. Instacart has a large, homogeneous grocery user base (18,739 users) with rich histories (10 baskets/user), creating favorable conditions for KNN: user similarity is reliable, and neighbor consensus amplifies the top-frequency item's score. At $K{=}1$, this frequency consensus is decisive. At larger $K$, however, the model must rank items with similar purchase counts but different timing — distinguishing items that are ``overdue'' from those just purchased. CASE's calendar-time CNN provides exactly this fine-grained cadence signal, which TIFUKNN's frequency aggregation cannot.

\textbf{TaFeng.} CASE wins across all $K$, and TIFUKNN underperforms even DNNTSP. With only 6.58 baskets per user, there is insufficient history to estimate reliable user neighborhoods, so TIFUKNN's neighbor aggregation degrades to noise. CASE is robust here because its CNN weights are shared and trained on all users simultaneously: even a user with 6 baskets benefits from population-level repurchase rhythms encoded during training, providing useful cadence features that individual-history methods cannot produce.

\textbf{DC.} CASE and TIFUKNN are nearly tied across all $K$. With only 852 distinct items and 1.60 baskets/user, the repurchase candidate set is tiny and trivially frequency-dominated — almost any method that identifies high-frequency items performs similarly. The per-user calendar vector $\mathbf{h}_i$ is nearly all zeros (at most 1--2 purchase events), so the CNN has little temporal pattern to exploit. The near-tie reflects a regime where the problem is effectively solved by frequency alone and cadence provides minimal marginal signal.

\textbf{Walmart.} CASE wins at $K{=}1,3,5$; TIFUKNN edges ahead at $K{=}10$. With 88,812 items and rich histories (11.72 baskets, 43 items/basket), many items share similar long-run frequencies, making cadence timing the primary discriminator at small $K$. The large basket size also gives ISAB rich cross-item structure to exploit: items co-purchased across 43-item baskets form meaningful clusters, and ISAB's induced-point attention captures these co-repurchase dependencies. At $K{=}10$, recall expands to cover more of the large test basket, and TIFUKNN's neighbor aggregation surfaces relevant items that may not yet register a timing signal in CASE's CNN.

\textbf{General pattern.} Across datasets, cadence timing is most decisive at small $K$ (precision-critical) and in dense, large-catalog settings. Neighbor-based KNN is most competitive when user pools are large and homogeneous and when $K$ is large enough that recall dominates. DNNTSP's basket-index encoding systematically loses the elapsed-time signal, causing it to underperform PersonalTop at $K{=}1$ on Instacart and Walmart — a concrete cost of discarding calendar time. PersonalTop, despite its simplicity, outperforms DNNTSP at low $K$ precisely because it implicitly encodes frequency, which is a coarse proxy for cadence. CASE subsumes both signals — cadence via CNN and co-purchase context via ISAB — and is the only approach that is simultaneously competitive offline and deployable at production scale.
